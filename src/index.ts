 ...
 *
 * Reads `procurement_bids`, populated daily
...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Gov Bids');
}
...
async function pg(cfg: Cfg, path: string, extraHeaders?: Record<string, string>): Promise<Response> {
  return pwFetch(`${cfg.url}/rest/v1/${path}`, {
...
async function jurisdictionsList(cfg: Cfg): Promise<unknown> {
  ...
  if (!res.ok) throw await httpError(res, 'bids store');
